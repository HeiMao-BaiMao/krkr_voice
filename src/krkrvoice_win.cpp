...
namespace
{
    ...
    static std::wstring GetGenderStringSAPI(CComPtr<ISpObjectToken> tok)
    {
        ...
        if (FAILED(tok->OpenKey(L"Attributes", &attr)) || !attr)
            return L"Unknown";
        WCHAR *v = nullptr;
                               ...
                               const std::wstring &genF)
    {
        std::wstring attr;
        if (!langF.empty())
            attr = L"Language=" + LocaleNameToHexLCID(langF);
        ...
        while (true)
        {
            ...
            std::wstring tag = L"?";
            {
                ...
                if (SUCCEEDED(tok->OpenKey(L"Attributes", &k)) && k)
                {
                    ...
                    if (SUCCEEDED(k->GetStringValue(L"Language", &v)) && v)
                    {
                        ...
                        if (::LCIDToLocaleName(id, buf, _countof(buf), 0) > 0)
                            tag = buf;
                        ::CoTaskMemFree(v);
                    }
                }
            ...
            if (!genF.empty() && _wcsicmp(g.c_str(), genF.c_str()) != 0)
                continue;

            WCHAR *desc = nullptr;
            SpGetDescription(tok, &desc);
                                ...
                                const std::wstring &genF)
    {
        ...
        {
            if (!langF.empty() && vi.Language() != langF)
                continue;

            std::wstring g = (vi.Gender() == SS::VoiceGender::Male)     ? L"Male"
                             : (vi.Gender() == SS::VoiceGender::Female) ? L"Female"
                                                                        : L"Other";
            if (!genF.empty() && _wcsicmp(g.c_str(), genF.c_str()) != 0)
                continue;
...
bool WinTTSService::SpeakText(const VoiceInfo &voice,
                              ...
                              std::shared_ptr<std::atomic_bool> cancelFlag)
{
    ...
    {
        CComPtr<ISpVoice> sp;
        sp.CoCreateInstance(CLSID_SpVoice);
        ...
        if (onFinish)
        {
            std::thread([sp, onFinish, cancelFlag]
                        {
        SPVOICESTATUS st{};
        while (true) {
            sp->GetStatus(&st, nullptr);
            if (st.dwRunningState == SPRS_DONE) break;
            if (cancelFlag && cancelFlag->load()) {
                sp->Speak(nullptr, SPF_PURGEBEFORESPEAK, nullptr); // 強制停止
                break;
            }
            ::Sleep(40);
        }
    ...
    if (voice.engine == L"WinRT")
    {
        ...
        { // クリティカル領域
            ...
            {
                if (!pl)
                    pl = g_singlePlayer = WP::MediaPlayer();
            ...
            pl.PlaybackSession().PlaybackStateChanged(
                [pl, &ended, onFinish](auto &&, auto &&)
                {
                    ...
                    {
                        if (!ended.exchange(true) && onFinish)
                            onFinish();
                    }
                });
            ...
            if (cancelFlag)
            {
                std::thread([pl, cancelFlag]
                            {
        while (pl && !cancelFlag->load()) {
            ::Sleep(40);
        }
        if (pl && cancelFlag->load()) {
            pl.Pause(); // または pl.Source(nullptr); で音源切断
        } })
                    .detach();
            }
        }
