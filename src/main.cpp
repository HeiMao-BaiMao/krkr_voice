...
static Options ParseArgs(int argc, wchar_t *argv[])
{
    ...
    if (opts.count("tts"))
        o.ttsType = WStringFromUTF8(opts["tts"]);
    if (opts.count("lang"))
        o.lang = WStringFromUTF8(opts["lang"]);
    if (opts.count("gender"))
        o.gender = WStringFromUTF8(opts["gender"]);
    ...
    if (opts.count("overlap"))
        o.overlap = true;
    if (opts.count("stop") || opts.count("replace"))
        o.overlap = false;
    if (opts.count("list"))
        o.listOnly = true;
