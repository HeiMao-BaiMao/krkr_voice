...
{
public:
    TTSToken() : flag_(std::make_shared<std::atomic_bool>(false)), cancelFlag_(nullptr) {}
    ...
    void setCancelFlag(std::shared_ptr<std::atomic_bool> flag) { cancelFlag_ = flag; } // ←これを追加

private:
    std::shared_ptr<std::atomic_bool> flag_;
    std::shared_ptr<std::atomic_bool> cancelFlag_; // ←これも宣言
};
...
public:
                       ...
                       tTJSVariant port = tTJSVariant())
    {
        ...
        if (!svc_)
            throw std::runtime_error("TTS service not available");
    }
    ...
    std::vector<std::wstring> list(const tjs_char *lang = L"", const tjs_char *gender = L"")
    {
        ...
        for (size_t i = 0; i < v.size(); ++i)
        {
            const auto &e = v[i];
            out.emplace_back(
                std::to_wstring(i) + L":" + e.displayName + L"," +
                e.engine + L"," +
                e.lang + L"," +
                e.gender);
        }
    ...
    TTSToken TTSBridge::speakSync(int idx, const tjs_char *lang, const tjs_char *gender,
                                  const tjs_char *text, int speed, bool overlap)
    {
        TTSToken tok;
        auto flag = tok.flag();
        auto v = svc_->GetVoiceList(lang, gender);
        ...
        tok.setCancelFlag(cancelFlag);

        svc_->SpeakText(v[idx], processed, speed, true, overlap, [flag]
                        { flag->store(true); }, cancelFlag);

        return tok;
    }

    TTSToken TTSBridge::speakAsync(int idx, const tjs_char *lang, const tjs_char *gender,
                                   const tjs_char *text, int speed, bool overlap)
    {
        TTSToken tok;
        auto flag = tok.flag();
        ...
        std::wstring processed = applyDictionary(text ? text : L"");
        // キャンセルフラグを用意してTTSTokenへセット
        auto cancelFlag = std::make_shared<std::atomic_bool>(false);
        tok.setCancelFlag(cancelFlag);
        // SpeakText呼び出しにcancelFlagを追加
        svc_->SpeakText(v[idx], processed, speed, false, overlap, [flag]
                        { flag->store(true); }, cancelFlag);
        return tok;
    }
    ...
    {
        if (enable)
            enabledDictionaries_.insert(name);
        else
            enabledDictionaries_.erase(name);
    }
